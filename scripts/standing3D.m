...
% ======================================================================
function problem = standing3D(model, resultfile, vaeParams)
...
%% VAE settings
useVAE = false;
if nargin >= 3 && ~isempty(vaeParams)
    useVAE = true;

    % Check VAE parameters
    requiredFields = {'modelPath', 'numDofs', 'latentDim'};
...
weightsType = 'volumeweighted';
exponent_muscle = 2;
problem.addObjective(@effortTermMusclesAct, Wact, weightsType, exponent_muscle);

% Torque objective
exponent_torque = 2;
problem.addObjective(@effortTermTorques, Wtor, exponent_torque);
