...
%> @retval  problem        Collocation: Generated standing problem
% ======================================================================
function problem = running3D(model,trackingData,initialFile,resultFile,N,sym,W, targetspeed_x, targetspeed_z, targetdur, vaeParams)

%% VAE settings
useVAE = false;
if nargin >= 11 && ~isempty(vaeParams)
    useVAE = true;
...
% Check dimension if VAE is enabled
%if useVAE
%    nJointsInModel = length(model.extractState('q'));
%    if nJointsInModel ~= vaeParams.numDofs + 6
%        error('Model has %d dofs but VAE expects %d', nJointsInModel, vaeParams.numDofs);
%    end
%end
...
if isinf(W.speed)
    ...
    problem.addOptimVar('speed', [targetspeed;-0.01], [targetspeed;0.01], [targetspeed;-0.007])
else
    problem.addOptimVar('speed',[-5;-5],[5;5], [1;1]); % Do not initialize with 0 to avoid errors in effortTermMuscles.m
end
% targetspeed_x: 4.001098 targetspeed_z: -0.007782 targetdur: 0.771250
...
if W.effMusclesTor ~= 0
    exponent_torque = 2;
    speedWeighting = 0;
    problem.addObjective(@effortTermTorques, W.effMusclesTor, exponent_torque, speedWeighting);
end
if W.cot ~= 0
    metabolic_model = 'bhargava';
    epsilon = 0.1;
    exponent_cot = 2;
    problem.addObjective(@cotTerm, W.cot, metabolic_model, epsilon, exponent_cot);
end
if W.track ~= 0
